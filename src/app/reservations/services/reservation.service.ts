...
// import { ApiService } from '../../shared/services/api2.service';
...
import { reservationsSelector, selectedDaySelector } from '../store/reservations.selectors';
...
export class ReservationService {
    ...
    updateReservation(id: string, payload: updateReservationInterface): Observable<ApiResponse> {
    ...
    addReservations(reservations: BookingModel) {
        ...
        cheking.length > 0 ? this.handleConflictReservations(cheking) : this.dispatchReservartions(reservations);
    }
    ...
    dispatchReservartions(reservations: BookingModel): void {
        ...
        for (let i = 0; i < repeatInterval; i++) {
            for (const place of reservations.places) {
                ...
                try {
                    this.store.dispatch(ReservationActions.addReservations({ reservation: newRes }));
                } catch (error) {
    ...
    isConflict(reservation: CreateReservationModel): boolean {
        ...
        return this.reservations.some(existingRes => {
            return (
                ...
                this.timeRangesOverlap(existingRes.startHour, existingRes.endHour, reservation.startHour, reservation.endHour) &&
                this.getIndexOfPossibleHours(reservation.startHour) <= this.getIndexOfPossibleHours(reservation.endHour)
            );
        });
    }
    ...
    getAvailableHours(date: string, place?: PlaceType): string[] {
        ...
        return this.dataService.availableHours.filter(hour => !reservedHours.includes(hour));
    }

    getReservedHours(date: string, place?: PlaceType): string[] {
        return this.reservations
            .filter(reservation => reservation.date === date && reservation.place === place)
            .flatMap(reservation => {
                const startIndex = this.dataService.availableHours.indexOf(reservation.startHour as TimeSlot);
                const endIndex = this.dataService.availableHours.indexOf(reservation.endHour as TimeSlot);
    ...
    getReservationsFromToDays(start: string, end: string, selectedFilters?: { label: string; group: string }[]): DaysReservationModel[] {
    ...
    private getAllReservationsFromDateRange(start: string, end: string): DaysReservationModel[] {
        ...
        while (currentDate < endDate) {
            ...
            const dailyReservations: ReservationModel[] = this.getReservationsByDay(formattedDate);
    ...
    private applyFilters(reservationsOfDays: DaysReservationModel[], selectedFilters: { label: string; group: string }[]): DaysReservationModel[] {
        return reservationsOfDays
            .map(day => {
                const filteredDayReservations = day.reservations.filter(reservation => {
    ...
    private createStatus(): ReservationStatus {
        return this.currentUser?.role === 'admin' ? ReservationStatus.APPROVED : ReservationStatus.PENDING;
    }
